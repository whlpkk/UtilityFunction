...
/*  eg： 实现下面的代理
   ...
   expansionSettings:(MGSwipeExpansionSettings *)expansionSettings;
   {
    ...
    if (direction == MGSwipeDirectionRightToLeft) {
        ...
        button.frame = CGRectMake(0, 0, 77, 100);
        button.titleLabel.font = [UIFont systemFontOfSize:15];
        [button setTitle:@"删除"];
        [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [button setBackgroundColor:[UIColor colorWithHexString:@"#ecb047"]];
   ...
   fromExpansion:(BOOL)fromExpansion
   {
    ...
    ManagerHouseModel *model = _dataArray[indexPath.row];
    [self deleteRoomRequestWithRoomId:model.roomId indexPath:indexPath];
    return YES;
   }
 */
