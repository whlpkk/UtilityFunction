...
-(id)initWithTitle:(NSString *)inTitle cancelButtonItem:(RIButtonItem *)inCancelButtonItem destructiveButtonItem:(RIButtonItem *)inDestructiveItem otherButtonItems:(NSArray *)otherButtonsArray
{
    if((self = [self initWithTitle:inTitle delegate:self cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:nil]))
    {
        ...
        if (otherButtonsArray && otherButtonsArray.count != 0)
        {
            [buttonsArray addObjectsFromArray:otherButtonsArray];
            for(RIButtonItem *item in otherButtonsArray)
            {
